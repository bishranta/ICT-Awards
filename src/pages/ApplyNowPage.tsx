    ...
    <div className="pt-20">
      ...
      <section className="bg-white section-padding text-center">
        <div className="container-max max-w-3xl">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-ink">
            Apply for <span className="text-gold">ICT Award 2026</span>
          </h1>
          <p className="mt-6 text-lg text-ink/70 leading-relaxed">
            Nominations open <strong className="text-gold">July 17, 2026</strong>. Applications are completely free — apply to any category you qualify for.
          </p>
          <div className="mt-8 inline-flex items-center gap-2 bg-gold/10 border border-gold/30 rounded-full px-5 py-2 text-gold text-sm font-semibold">
            ...
            {ICT_2026_TIMELINE.map((event, idx) => (
              <div
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border ${
                  event.highlight ? 'bg-gold/10 border-gold/30' : 'bg-white border-gray-100'
                }`}
              >
                <div className={`w-3 h-3 rounded-full flex-shrink-0 ${event.highlight ? 'bg-gold' : 'bg-gray-300'}`} />
                <div className="flex-1">
                  <div className={`font-bold ${event.highlight ? 'text-gold' : 'text-ink'}`}>{event.label}</div>
                  <div className="text-ink/50 text-sm">{event.date}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="bg-surface-alt section-padding">
        <div className="container-max">
          ...
          <p className="text-ink/60 mb-8">Choose the category that best matches your achievement, innovation, or contribution.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {CATEGORY_GROUPS.map((group) => {
              const cats = CATEGORIES.filter(c => c.group === group.group)
              return (
                <div key={group.group} className="bg-white border border-gray-100 rounded-xl p-6">
                  <div className="text-xs text-gold/70 font-bold uppercase tracking-widest mb-1">Category {group.group}</div>
      ...
      <section className="bg-gold section-padding text-center">
        <div className="container-max max-w-2xl">
          ...
          <p className="mt-6 text-sm text-ink/60">Have questions? Visit our <Link to="/about#faq" className="underline font-semibold">FAQ page</Link></p>
        </div>
      </section>
    </div>
  )
