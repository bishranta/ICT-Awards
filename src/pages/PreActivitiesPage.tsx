        ...
        <div className="container-max">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {preEvents.map((activity) => (
              <Link key={activity.id} to={`/pre-activities/${activity.id}`} className="block hover:scale-[1.01] transition-transform duration-200">
                <div className="gold-border-card h-full">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-ink/80 leading-relaxed max-h-[8.125em] overflow-y-auto pr-1 scrollbar-subtle">{activity.description}</p>
                    </div>
                  </div>
                </div>
              </Link>
        ...
        <div className="container-max">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {otherActivities.map((activity) => (
              <Link key={activity.id} to={`/pre-activities/${activity.id}`} className="block hover:scale-[1.01] transition-transform duration-200">
                <div className="bg-surface-alt border border-border-subtle hover:border-gold/30 rounded-xl p-6 transition-all h-full">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl bg-gold/10 flex items-center justify-center flex-shrink-0">
                      <RenderIcon name={activity.icon} size={24} />
                    </div>
