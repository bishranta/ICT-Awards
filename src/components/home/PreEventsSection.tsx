import { Link } from 'react-router-dom'
import { ArrowRight } from '@phosphor-icons/react'
import { PRE_ACTIVITIES } from '@/data/preActivities'
import { RenderIcon } from '@/lib/renderIcon'
import SectionHeading from '@/components/ui/SectionHeading'

export default function PreEventsSection() {
  const featured = PRE_ACTIVITIES.slice(0, 4)

  return (
    <section className="bg-cream section-padding">
      <div className="container-max">
        <div className="relative mb-10">
          <SectionHeading
            title="Pre-Award Activities"
            subtitle="An ecosystem of events, mentoring, and outreach that runs alongside the award selection process."
            centered
          />
          <Link to="/pre-activities" className="mt-4 flex justify-center text-spectrum-a font-bold hover:text-ink items-center gap-2 whitespace-nowrap md:mt-0 md:absolute md:top-0 md:right-0">
            More Pre-Events
            <ArrowRight size={16} className="icon-spectrum" />
          </Link>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {featured.map((activity) => (
            <div key={activity.id} className="bg-surface rounded-xl p-6 shadow-sm hover:shadow-md border border-border-subtle hover:border-gold/30 transition-all group">
              <div className="w-10 h-10 rounded-lg bg-gold/10 flex items-center justify-center mb-3">
                <RenderIcon name={activity.icon} size={20} />
              </div>
              <h3 className="font-bold text-ink text-lg mb-2 group-hover:text-spectrum-a transition-colors">
                {activity.title}
              </h3>
              <p className="text-ink/80 text-sm leading-relaxed">
                {activity.description.slice(0, 100)}…
              </p>
            </div>
          ))}
        </div>
      </div>
    </section>
  )
}
